...
function EqF9▬6(M_p, M_y, L_b, L_p, L_r)
    M_nLTB = (M_p - (M_p-M_y)*((L_b-L_p)/(L_r-L_p)))
end
...
function EqF9▬9(E, F_y, I_y, J, S_x, d)
    L_r = 1.95 * (E/F_y) * sqrt(I_y*J)/S_x * sqrt(2.36 * (F_y/E) * (d*S_x)/J + 1)
end

function EqF9▬10(E, L_b, I_y, J, B)
    M_cr = (1.95*E)/L_b * sqrt(I_y*J) * (B + sqrt(1 + B^2))
end
...
function EqF9▬14(M_p, F_y, S_xc, λ_f, λ_pf, λ_rf, M_y)
    M_nFLB1 = (M_p - (M_p-F_y*S_xc)*((λ_f-λ_pf)/(λ_rf-λ_pf)))
    M_nFLB2 = 1.6*M_y
...
function EqF9▬18(d, t_w, F_y, E)
    F_cr = (1.43 - 0.515*(d/t_w)*sqrt(F_y/E)) * F_y
end
