...
function EqF2▬4(C_b, E, L_b, r_ts, J, c, S_x, h_0)
    F_cr = ((C_b*π^2*E)/(L_b/r_ts)^2)*sqrt(1 + 0.078*((J*c)/(S_x*h_0))*(L_b/r_ts)^2) |> cnv.to_stress
end
...
function EqF2▬6(r_ts, E, F_y, J, c, S_x, h_0)
    L_r = 1.95*r_ts*(E/(0.7*F_y))*sqrt((J*c)/(S_x*h_0) + sqrt(((J*c)/(S_x*h_0))^2 + 6.76*((0.7*F_y)/E)^2)) |> cnv.to_large_L
end

function EqF2▬7(I_y, C_w, S_x)
    r_ts = sqrt(sqrt(I_y*C_w)/S_x) |> cnv.to_L
end
